    ...
    <div className='w-52 h-16 flex flex-col rounded-lg overflow-hidden shadow-lg bg-gray-100'>
      <div className='h-1/2 flex flex-row items-center justify-between'>
        <div className='h-full flex flex-row items-center'>
          <ColorButton/>
          <ColorButton color={HighlightColor.Red}/>
          <ColorButton color={HighlightColor.Orange}/>
          <ColorButton color={HighlightColor.Yellow}/>
          <ColorButton color={HighlightColor.Lime}/>
          <ColorButton color={HighlightColor.Cyan}/>
          <ColorButton color={HighlightColor.Indigo}/>
      </div>
        <div className='h-full p-1.5 flex justify-center items-center border-l text-base'>
          <Underline active/>
        </div>
      </div>
      <div className='h-1/2 border-t'>
        <input type='text' className='block h-full w-full outline-none p-1'/>
      </div>
    </div>
  )
}
...
function Underline({ active = false, color = HighlightColor.None }) {
  const colorClass = color ? `fill-${color}-400` : `fill-gray-300`
  return (
